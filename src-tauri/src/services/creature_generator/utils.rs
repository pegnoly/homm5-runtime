...
pub async fn process_generation(
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Eof) => break Ok(String::from_utf8(output)?),
            Ok(Event::Start(e)) => {
                ...
                match actual_tag.as_str() {
                    "BaseCreature" => {
                        ...
                        if creature_model.parent_creature.is_some() {
                            writer.create_element("BaseCreature").write_text_content(
                                BytesText::new(
                                    &creatures_data
                                        .iter()
                            ...
                            writer
                                .create_element("BaseCreature")
                                .write_text_content(BytesText::new("CREATURE_UNKNOWN"))?;
                        }
                    }
                    "PairCreature" => {
                        let end = e.to_end().into_owned();
                    ...
                    "SubjectOfRandomGeneration" => {
                        ...
                        reader.read_to_end(end.name())?;
                        writer
                            .create_element("SubjectOfRandomGeneration")
                            .write_text_content(BytesText::new("false"))?;
                    }
                    "Upgrades" => {
                        let end = e.to_end().into_owned();
                        let text = reader.read_text(end.name())?.to_string();
                        let mut upgrades = quick_xml::de::from_str::<Upgrades>(&format!(
                            "<Upgrades>{}</Upgrades>",
                            &text
                        ))?;
                        if !creature_model.upgrades.is_empty() {
                            upgrades.upgrages =
                                Some(Vec::from_iter(creature_model.upgrades.iter().map(|u| {
                                    creatures_data
                                        .iter()
                                        .find(|cr| cr.id == *u)
                                        .unwrap()
                    ...
                    "Abilities" => {
                        let end = e.to_end().into_owned();
                        let text = reader.read_text(end.name())?.to_string();
                        let mut abilities = quick_xml::de::from_str::<Abilities>(&format!(
                            "<Abilities>{}</Abilities>",
                            &text
                        ))?;
                        if !selected_abilities.is_empty() {
                            ...
                            for ability in selected_abilities {
                                abilities.Abilities.as_mut().unwrap().push(
                                    abilities_data
                                        .iter()
                                        .find(|a| a.id == *ability)
                                        .unwrap()
                    ...
                    _ => {
                        ...
                        if !inner_name_created {
                            ...
                            writer.create_element("InnerName").write_text_content(
                                BytesText::new(creature_model.inner_name.as_ref().unwrap()),
                            )?;
                        }
                    }
                }
