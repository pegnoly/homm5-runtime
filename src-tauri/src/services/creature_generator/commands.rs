use crate::{
    error::Error,
    services::creature_generator::{
        types::{CreatableCreatureModel, CreatureGeneratorSessionConfig},
        utils::process_generation,
    },
...
pub async fn generate_creatures(
    ...
    for model in &models {
        let base_creature_data = scaner_repo
            .get_creature(model.base_creature.unwrap())
            .await?
            .unwrap();
        let result = process_generation(
            &creatures_data,
            &abilities_data,
            &base_creature_data,
            model,
...
#[tauri::command]
pub async fn pick_session_file(app: AppHandle) -> Result<(), Error> {
    app.dialog()
        .file()
        .add_filter("Session files", &["json"])
        .pick_file(move |f| app.emit("session_file_picked", f).unwrap());
    Ok(())
}

#[tauri::command]
pub async fn load_session(session_file: String) -> Result<CreatureGeneratorSessionConfig, Error> {
