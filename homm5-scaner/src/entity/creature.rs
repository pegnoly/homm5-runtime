use super::{CollectFiles, FileObjects, FileRef, FileStructure, Output, Scan, configure_path};
use homm5_types::creature::{AdvMapCreatureShared, CreatureVisual};
...
impl Output for AdvMapCreatureShared {
    ...
    fn to_lua(&self, id: &Option<u16>) -> String {
        ...
        let is_upgrade = if self.BaseCreature.is_some()
            && self.BaseCreature.as_ref().unwrap() != "CREATURE_UNKNOWN"
        {
            "1"
        } else {
            "nil "
