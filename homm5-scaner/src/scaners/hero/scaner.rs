    ...
    fn scan(
        ...
        loop {
            match reader.read_event_into(&mut buf) {
                ...
                Ok(Event::Start(e)) => if e.name().as_ref() == b"AdvMapHeroShared" {
                    let end = e.to_end().into_owned();
                    let possible_text = reader.read_text(end.name());
                    match possible_text {
                        Ok(text) => {
                            let text = text.to_string();
                            let de_res: Result<AdvMapHeroShared, quick_xml::DeError> = quick_xml::de::from_str(&format!("<AdvMapHeroShared>{text}</AdvMapHeroShared>"));
                            match de_res {
                                Ok(mut hero) => {
                                    self.id = file_key.to_string();
                        ...
                        Err(e) => println!("error reading file content: {file_key}, {e}"),
                    }
                },
                _ => (),
            }
