...
use crate::{
    ...
    pak::FileStructure,
    scaners::{
        skill::model::{Model, NamePaths, Names},
        types_scaner::GameTypeItem,
    },
        ...
        match skill_de {
            Ok(skill) => {
                let actual_name_paths = if let Some(paths) = skill.obj.NameFileRef.names {
                    paths
                        .iter()
                        .map(|p| configure_path(p.href.as_ref(), file_key, files))
                        .collect_vec()
                } else {
                    vec![]
                };
                let actual_names = actual_name_paths
                    .iter()
                    .filter_map(|n| files.get(n).map(|name| name.content.clone()))
                    .collect_vec();
                self.id += 1;
                let model = Model {
                    id: self.id,
