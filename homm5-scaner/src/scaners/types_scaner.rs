...
impl TypesXmlScaner {
    pub fn parse(&mut self, files: &HashMap<String, FileStructure>) -> Result<(), ScanerError> {
        let types_xml = files
            .iter()
            .find(|f| f.0 == "types.xml".to_lowercase().as_str())
            .unwrap();

        let document = roxmltree::Document::parse(&types_xml.1.content).unwrap();
        let creatures_elem = document
            .descendants()
        ...
        let creature_entries_node = creatures_elem
            .descendants()
        ...
        creature_entries_node.descendants().for_each(|n| {
            if n.tag_name() == "Name".into() {
                ...
                self.creature_items.push(GameTypeItem {
                    name: name.clone(),
                    value,
                });
            }
        ...
        let skills_entries_node = skills_elem
            .descendants()
            .find(|n| n.tag_name() == "Entries".into())
            .unwrap();
        let mut name = String::new();
        ...
        skills_entries_node.descendants().for_each(|n| {
            if n.tag_name() == "Name".into() {
                ...
                value = n.text().unwrap().parse::<i32>().unwrap();
                self.skills_items.push(GameTypeItem {
                    name: name.clone(),
                    value,
                });
            }
        ...
        let spells_entries_node = spell_elem
            .descendants()
            .find(|n| n.tag_name() == "Entries".into())
            .unwrap();
        let mut name = String::new();
        ...
        spells_entries_node.descendants().for_each(|n| {
            if n.tag_name() == "Name".into() {
                ...
                value = n.text().unwrap().parse::<i32>().unwrap();
                self.spells_items.push(GameTypeItem {
                    name: name.clone(),
                    value,
                });
            }
