...
use homm5_types::{
    common::{ArmySlot, FileRef},
    hero::{AdvMapHero, Editable, FavoriteEnemies, Perks, Skill, Skills, SpellIds},
    player::PlayerID,
    town::ArmySlots,
};
...
impl From<Model> for AdvMapHero {
    fn from(value: Model) -> Self {
        AdvMapHero {
            ...
            army_slots: Some(ArmySlots {
                army_slots: Some(vec![{
                    ArmySlot {
                        creature: String::from("CREATURE_1000"),
                        count: 1,
                    }
                }]),
            }),
            editable: Editable {
                skills: Some(Skills {
                    items: Some(Vec::from_iter(value.skills.skills.iter().map(|s| Skill {
                        Mastery: s.mastery.to_string(),
                        SkillID: s.skill.clone(),
                    }))),
                }),
                perkIDs: Some(Perks {
                    items: Some(Vec::from_iter(
                        value.skills.skills.iter().flat_map(|s| s.perks.clone()),
                    )),
                }),
                spellIDs: Some(SpellIds {
                    items: Some(Vec::from_iter(value.spells.spells.iter().cloned())),
                }),
                FavoriteEnemies: if value.favorite_enemies.enemies.is_empty() {
                    None
                } else {
                    Some(FavoriteEnemies {
                        items: Some(value.favorite_enemies.enemies),
                    })
                },
                ..Default::default()
            },
            primary_skill_mastery: value
                .skills
                .skills
                .iter()
                .find(|s| s.slot == 0)
                ...
                .to_string(),
            shared: FileRef {
                href: Some(format!("/{}#xpointer(/AdvMapHeroShared)", &value.xdb_path)),
            },
            ..Default::default()
        }
    }
