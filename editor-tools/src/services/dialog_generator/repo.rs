...
use crate::{error::EditorToolsError, prelude::{CreateDialogPayload, CreateSpeakerPayload, DialogModel, DialogVariantModel, GetVariantPayload, SaveVariantPayload, SpeakerModel, UpdateLabelsPayload}, services::dialog_generator::{models::{dialog::{self, Labels, SpeakersIds}, speaker, variant}, prelude::CreateVariantPayload}};
...
impl DialogGeneratorRepo {
    ...
    pub async fn load_dialogs(&self, mission_id: i32) -> Result<Vec<DialogModel>, EditorToolsError> {
        Ok(dialog::Entity::find().filter(dialog::Column::MissionId.eq(mission_id)).all(&self.db).await?)
    }
    ...
    pub async fn create_dialog(&self, payload: CreateDialogPayload) -> Result<DialogModel, EditorToolsError> {
        let model_to_insert = dialog::ActiveModel {
            ...
            labels: Set(Labels { labels: vec!["main".to_string()] }),
            speakers_ids: Set(SpeakersIds { ids: payload.speakers }),
    ...
    pub async fn create_speaker(&self, payload: CreateSpeakerPayload) -> Result<SpeakerModel, EditorToolsError> {
    ...
    pub async fn get_variant(&self, payload: GetVariantPayload) -> Result<Option<DialogVariantModel>, EditorToolsError> {
        Ok(
            variant::Entity::find()
    ...
    pub async fn update_dialog_labels(&self, payload: UpdateLabelsPayload) -> Result<(), EditorToolsError> {
        if let Some(existing_dialog) = dialog::Entity::find_by_id(payload.dialog_id).one(&self.db).await? {
            let mut model_to_update = existing_dialog.into_active_model();
            model_to_update.labels = Set(Labels { labels: payload.labels });
            model_to_update.update(&self.db).await?;
        }
        Ok(())
    }

    pub async fn create_variant(&self, payload: CreateVariantPayload) -> Result<DialogVariantModel, EditorToolsError> {
    ...
    pub async fn save_variant(&self, payload: SaveVariantPayload) -> Result<(), EditorToolsError> {
        if let Some(existing_variant) = variant::Entity::find_by_id(payload.id).one(&self.db).await? {
            let mut model_to_update = existing_variant.into_active_model();
            model_to_update.text = Set(payload.text);
            model_to_update.speaker_id = Set(payload.speaker);
            model_to_update.update(&self.db).await?;
        }
        Ok(())
    }
