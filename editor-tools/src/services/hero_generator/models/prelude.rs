pub use super::asset::Model as HeroAssetModel;
pub use super::artifacts::Model as HeroAssetArtifactsModel;
pub use super::army_slot::{
    Model as HeroAssetArmySlotModel, 
    ArmyGenerationRuleParam,
    ArmySlotStackCountGenerationMode,
    ArmySlotStackUnitGenerationMode
};
pub use super::stat_generation::{
    Model as ArmyStatGenerationModel,
    ArmyGenerationStatParam,
    ArmyGenerationStatRule
};